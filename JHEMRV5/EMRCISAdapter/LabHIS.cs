...
{
    public partial class LabHIS : UserControl, EmrEditUCInterface
    {
        ...
        void EmrEditUCInterface.setPatientInfo(string strPatientID, int nVisitID)
        {
            ...
            DataSet LabHIS = new DataSet();
            LabHIS = DALUse.Query(strSQLl);
            if (LabHIS.Tables[0].Rows[0]["HISORDER"].ToString() == EmrSysPubVar.getDbUser())
            {
                inp_no = Convert.ToInt32(LabHIS.Tables[0].Rows[0]["inp_no"]);
        ...
        private void LabHIS_Load(object sender, EventArgs e)
        {
